...
/*   Updated: 2017/03/13 22:27:16 by fsidler          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void            Game::_gameLoop()
{
    ...
    while ((key = wgetch(_mainWin)) != KEY_ESC && _playerList && _timer > 0 && (!bossPop || _bossList))
    {
        ...
        if (_timer == 100)
        {
            pushInList(_bonusList, new Bonus("$", new Weapon(bonusWeap), (t_coord){40, 10}));
            pushInList(_bonusList, new Bonus("$", new Weapon(bonusWeap), (t_coord){80, 50}));
        }
        _gameCore(key);
        wrefresh(_mainWin);
